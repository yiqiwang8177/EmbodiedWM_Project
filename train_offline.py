...
def train_eval(config):
    ...
    config.dp["pretrained_ckpt"] = config.pretrained_base_ckpt
    ...
    if config.train_dp_mppi:
        ...
        # 0. train the Dreamer World Model (WM) offline with the expert dataset.
...
if __name__ == "__main__":
    ...
    configs = yaml.load(
        (pathlib.Path(sys.argv[0]).parent / "sailor/offline_configs.yaml").read_text()
    )
