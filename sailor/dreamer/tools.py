...
class Logger:
    ...
    def histogram(self, name, np_hist):
        self._histograms[name] = np_hist

    def write(self, step, fps=False, flush=False):
        scalars = list(self._scalars.items())
        if fps:
            scalars.append(("fps", self._compute_fps(step)))

        # Log scalars to wandb
        wandb.log({name: value for name, value in scalars}, step=step)
        ...
        for name, np_hist in self._histograms.items():
            wandb.log({name: wandb.Histogram(np_histogram=np_hist)}, step=step)

        # Commit metrics wandb
        if flush:
            wandb.log({"commit": True}, step=step, commit=True)
    ...
    if reverse:
        ...
        for filename in sorted(directory.glob("*.npz")):
            ...
            if limit and total >= limit:
                break
    cprint(
        f"Loaded {total_eps} episodes with total transitions: {total} from {directory}",
        "green",
    )
    return episodes
...
class DiscDist:
    def __init__(
        ...
        self.buckets = transbwd(torch.linspace(low, high, steps=255)).to(device)
        self.transfwd = transfwd
        self.transbwd = transbwd

    def mean(self):
        _mean = self.probs * self.buckets
        return torch.sum(_mean, dim=-1, keepdim=True)
    ...
    def reduce_over_dist(cls, dist1, dist2, comparison_fn):
        ...
        # Assert both distributions have same buckets
        assert torch.allclose(dist1.buckets, dist2.buckets), (
            dist1.buckets,
            dist2.buckets,
        )
...
class ContDist:
    ...
    @classmethod
    def take_mean(cls, dists):
        """
        Create a new ContDist with the mean of all distributions in `dists`.
        Asserts that all distributions have the same `absmax` and type.
        """
        # Assert all distributions are of the same type
        dist_types = [type(dist._dist) for dist in dists]
        assert all(
            dtype == dist_types[0] for dtype in dist_types
        ), "All distributions must have the same type."

        # Assert all `absmax` values are the same
        absmax_values = [dist.absmax for dist in dists]
        assert all(
            absmax == absmax_values[0] for absmax in absmax_values
        ), "All `absmax` values must be the same."

        # Stack the means of all distributions
        means = torch.stack([dist.mean for dist in dists])
        # Compute the mean across all distributions
        mean_of_means = means.mean(dim=0)

        # Assuming all distributions are Independent(Normal(...), ...) for simplicity
        # Get the scale (stddev) from the first distribution
        scale = dists[0]._dist.base_dist.scale

        # Create a new Independent(Normal(...), ...) distribution
        new_dist = Independent(Normal(loc=mean_of_means, scale=scale), 1)

        # Return a new ContDist object
        return cls(new_dist, absmax=absmax_values[0])

    @classmethod
    def reduce_over_dist(cls, dist1, dist2, comparison_fn):
        """
        Create a new ContDist that is the minimum of two distributions
        based on some criterion on the mode.
        """
        assert (
            dist1._dist.base_dist.loc.shape == dist2._dist.base_dist.loc.shape
        ), "The two distributions must have the same shape"

        # Assert both distributions have same absmax
        assert dist1.absmax == dist2.absmax, (dist1.absmax, dist2.absmax)

        mode1 = dist1.mode()
        mode2 = dist2.mode()
        mask = comparison_fn(mode1, mode2)
        new_loc = torch.where(
            mask, dist1._dist.base_dist.loc, dist2._dist.base_dist.loc
        )
        # Create a new ContDist instance with the new loc
        combined_dist = cls(
            Independent(Normal(loc=new_loc, scale=dist1._dist.base_dist.scale), 1),
            absmax=dist1.absmax,
        )
        return combined_dist
