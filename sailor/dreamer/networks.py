...
class RSSM(nn.Module):
    ...
    def observe(self, embed, action, is_first, state=None):
        ...
        # NOTE:
        # Our buffers store (S_0, A_0) , (S_1, A_1) ... (S_t-1, A_t-1)
        # Dreamer simulate buffer stores (S_0, Null) , (S_1, A_0) ... (S_t-1, A_t-2) and static_scan assumes that format
        # Insert a zero in front of the action tensor to make data consistent with simulate buffer
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
        ...
        if prev_state == None or torch.sum(is_first) == len(is_first):
            ...
            prev_action = torch.zeros(
                (len(is_first), self._num_actions, self._pred_horizon),
                device=self._device,
            )

        if len(prev_action.shape) == 3:  # greedy first action selection
...
class MultiEncoder(nn.Module):
    def __init__(
        ...
        if "embedding" in shapes:
            # Remove the hardcoder 2 later -> it should be obschunk
            self.outdim += shapes["embedding"][0]
    ...
    def forward(self, obs):
        ...
        # Add embedding as is to the output
        if "embedding" in obs and "embedding" not in self.mlp_shapes:
            if self.symlog_inputs:
                outputs.append(tools.symlog(obs["embedding"]))
            else:
                outputs.append(obs["embedding"])
...
class MLPEnsemble(nn.Module):
    ...
    def forward_dist(self, features):
        """
        Take the min of the selected models' outputs.
        """
        # Randomly select num_subsample models
    ...
    @torch.no_grad()
    def forward(self, features):
        # Features shape BS x BL x ...

        # Randomly sample num_subsample models to use
        idxs = np.random.choice(len(self.models), self.num_subsample, replace=False)

        # Get model outputs at idxs (mode shape: BS x ...)
        idx_values = [
            self.models[idx](features) for idx in idxs
        ]  # num_subsample x BS x BL x ...

        mean_dist = tools.ContDist.take_mean(idx_values)
        return mean_dist

    def get_all_critic_mean(self, features):
        mean_dist = tools.ContDist.take_mean([model(features) for model in self.models])
        return mean_dist

    def update(self, features, target):
        all_values = [model(features) for model in self.models]
        value_losses = torch.stack(
            [-value.log_prob(target.detach()) for value in all_values]
        )  # num_models x BS x BL x ...

        # Take mean accross all dims except num_models
        mean_loss = value_losses.mean(dim=0)
        return mean_loss

    def get_std(self, features):
        all_value_mode = torch.stack([model(features).mode() for model in self.models])

        if all_value_mode.shape[0] <= 1:  # Check size along axis=0
            ensemble_std = torch.zeros_like(all_value_mode)[0, ...]
        else:
            ensemble_std = torch.std(all_value_mode, axis=0, unbiased=False)

        return ensemble_std

    def get_stats(self, features):
        all_value_mode = torch.stack(
            [model(features).mode() for model in self.models]
        )  # num_models x BS x BL x ...

        if all_value_mode.shape[0] <= 1:  # Check size along axis=0
            ensemble_std = torch.zeros_like(
                all_value_mode
            )  # Return a tensor of zeros with the same shape
        else:
            ensemble_std = torch.std(
                all_value_mode, axis=0, unbiased=False
            )  # Use unbiased=False for safety

        return_dict = {
            "ensemble_std_avg": torch.mean(ensemble_std).item(),
            "ensemble_std_max": torch.max(ensemble_std).item(),
            "ensemble_std_min": torch.min(ensemble_std).item(),
        }
        return return_dict
