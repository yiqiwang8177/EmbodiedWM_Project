...
class DiffusionUnetAgent(Agent):
    def __init__(
        ...
        self.noise_net = ConditionalUnet1D(
            input_dim=ac_dim, global_cond_dim=self.obs_enc_dim, **noise_net_kwargs
        )
        ...
        self.ema = EMAModel(parameters=self.noise_net.parameters(), decay=0.75)

    def copy_ema_to_model(self):
        # Store current training params in EMA
        self.ema.store(self.noise_net.parameters())

        # Load EMA params to model
        self.ema.copy_to(self.noise_net.parameters())

        print("Storing model weights params and loading EMA to model.")

    def restore_model_from_ema(self):
        # If weights are stored in EMA, restore them to model
        if self.ema.temp_stored_params is not None:
            self.ema.restore(self.noise_net.parameters())
            print("Restoring model weights from EMA.")
    ...
    def debug_batch(self, img, obs, savestr="train"):
        ...
        # breakpoint()
        img0 = img["cam0"][0, 0].cpu().numpy().transpose(1, 2, 0)
        plt.imshow(img0)
        plt.savefig(f"/home/dreamerv3/{savestr}_img0.png")
        plt.close()
        plt.clf()

        img1 = img["cam1"][0, 0].cpu().numpy().transpose(1, 2, 0)
        plt.imshow(img1)
        plt.savefig(f"/home/dreamerv3/{savestr}_img1.png")
        plt.close()
        plt.clf()

        # print min and max of obs
    ...
    def get_actions(self, imgs, obs, scale=1.0, n_steps=None):
        ...
        for timestep in self.diffusion_schedule.timesteps:
            ...
            noise_pred = scale * self.noise_net(noise_actions, batched_timestep, s_t)
