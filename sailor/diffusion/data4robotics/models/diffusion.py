...
class MLPResFILMBlock(nn.Module):
    def __init__(self, dim, cond_dim, dropout=0.1, layer_norm=True, alpha=2):
        ...
        self.film_embed = nn.Sequential(nn.Linear(cond_dim, alpha * dim), nn.ReLU())
        self.out_layers = nn.Sequential(nn.Linear(alpha * dim, dim), nn.ReLU())
...
class NoiseNetwork(nn.Module):
    def __init__(
        ...
        net = [
            MLPResFILMBlock(hidden_dim, in_dim, dropout, use_layer_norm)
